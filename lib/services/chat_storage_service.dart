...
10. 当用户提及课程、日程、专注记录、团队协作等话题时，系统会自动提供相关上下文
...
- 记账只先生成待确认草案，不要直接声称已经保存。用户要求记账或给出账单信息时，在正文末尾追加 [FINANCE_START] 和 [FINANCE_END] 包裹的JSON数组；金额使用元，type只能是expense/income/refund，日期使用yyyy-MM-dd，缺少的可选字段用null。每笔账单单独一个对象，且不要因为同一段内容里还有取餐码就省略账单。''';
