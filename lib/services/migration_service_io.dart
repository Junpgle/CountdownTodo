...
class MigrationService {
  ...
  static Future<void> runMigration({
    ...
    try {
      ...
      if (loginRes['success'] != true) {
        throw Exception(
            "旧服务器登录失败: ${loginRes['message'] ?? loginRes['error']}");
      }
      onProgress("✅ 登录旧服务器成功，开始拉取完整数据...");

      final Map<String, dynamic> user =
          loginRes['user'] is Map<String, dynamic> ? loginRes['user'] : {};
      ...
      try {
        pomodoroRecords = await ApiService.fetchPomodoroRecords(oldUserId);
      } catch (_) {}
      ...
      if (registerRes['success'] != true) {
        final errorMsg =
            registerRes['message'] ?? registerRes['error'] ?? '未知错误';
      ...
      // 兼容两种返回格式
      final int newUserId =
          registerRes['user_id'] ?? registerRes['user']?['id'] ?? 0;
      ...
      if (oldUserId != newUserId) {
        ...
        await _migrateLocalDataUserId(
            oldUserId.toString(), newUserId.toString());
      }
      ...
      if (pomodoroRecords.isNotEmpty) {
        ...
        final updatedRecords =
            pomodoroRecords.cast<Map<String, dynamic>>().map((r) {
          r['user_id'] = newUserId;
          return r;
        }).toList();
  ...
  static Future<void> _migrateLocalDataUserId(
