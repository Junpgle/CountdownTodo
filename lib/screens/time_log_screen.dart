...
class _TC {
  ...
  static Color timeLabel(BuildContext ctx, {bool major = false}) =>
      major
          ? (Theme.of(ctx).brightness == Brightness.dark ? const Color(0xFF666666) : const Color(0xFF888888))
          : (Theme.of(ctx).brightness == Brightness.dark ? const Color(0xFF2C2C2C) : const Color(0xFFCCCCCC));
}
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: _isLoading
          ...
          tags: _tags,
          onDayTap: (d) => setState(() { _focusedDate = d; _view = _View.day; }),
          onTagTap: _showTagDetail,
          onManageTags: _showTagManager,
          onAddLog: () => setState(() { _focusedDate = DateTime.now(); _view = _View.day; }),
          ...
          onSaveLog: (log) { _addLog(log); setState(() => _view = _View.week); },
          onPomodoroTap: _showPomodoroDetail,
        ),
      ),
    );
  ...
  void _showPomodoroDetail(PomodoroRecord pom) {
    ...
    final tag = pom.tagUuids.isNotEmpty
        ? _tags.cast<PomodoroTag?>().firstWhere(
            (t) => t?.uuid == pom.tagUuids.first, orElse: () => null)
        : null;
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(top: BorderSide(color: Colors.redAccent.withOpacity(0.4), width: 1.5)),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Text('专注记录', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: _TC.text(context))),
            const Spacer(),
            Container(
            ...
            IconButton(icon: Icon(Icons.close, size: 18, color: _TC.textHint(context)),
          ...
          if (tag != null) ...[
            ...
            Row(children: [
              ...
              Text(tag.name, style: TextStyle(fontSize: 13, color: tc, fontWeight: FontWeight.w600)),
            ]),
          ],
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: _InfoCard(label: '开始', value: DateFormat('HH:mm').format(startDt), sub: DateFormat('MM/dd').format(startDt))),
            const SizedBox(width: 10),
            Expanded(child: _InfoCard(label: '结束', value: DateFormat('HH:mm').format(endDt), sub: DateFormat('MM/dd').format(endDt))),
            const SizedBox(width: 10),
            Expanded(child: _InfoCard(label: '时长', value: '${dur}min', sub: (pom.isCompleted == true) ? '✓ 完成' : '手动停止')),
          ]),
        ]),
      ),
    );
  ...
  void _showTagDetail(PomodoroTag tag) {
    showModalBottomSheet(
      ...
      builder: (_) => _TagDetailSheet(
        tag: tag, logs: _allLogs, pomodoros: _allPomodoros,
        onDelete: (id) { Navigator.pop(context); _deleteLog(id); },
      ),
    );
  }
...
class _WeekView extends StatefulWidget {
  ...
  const _WeekView({
    required this.logs, required this.pomodoros, required this.tags,
    required this.onDayTap, required this.onTagTap, required this.onManageTags,
    required this.onAddLog, required this.onPomodoroTap, required this.username,
  });
...
class _WeekViewState extends State<_WeekView> {
  ...
  Widget build(BuildContext context) {
    ...
    final todayPomMin = widget.pomodoros
        .where((p) => p.startTime < todayEndMs &&
    ...
    return LayoutBuilder(builder: (ctx, outerConstraints) {
      ...
      Widget? sidebar = isWide ? _buildTagSidebar(days, weekStart, weekEnd) : null;
  ...
  // ── 网格主体（日期头 + 时间轴 + 7列）──────────────────
  Widget _buildGrid(BuildContext ctx, List<DateTime> days, int todayMin, double availW) {
    ...
    int _dayTotalMin(DateTime d) {
      ...
      final pomMin = widget.pomodoros
          .where((p) => p.startTime < de && (p.endTime ?? p.startTime + p.effectiveDuration * 1000) > ds)
          .fold(0, (s, p) {
              ...
              child: Container(
                ...
                child: FittedBox(
                  ...
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text('${d.day}', style: TextStyle(fontSize: 15,
                        color: isToday ? _TC.text(ctx) : _TC.textSub(ctx),
                        fontWeight: isToday ? FontWeight.w700 : FontWeight.w400)),
                    if (dayMin > 0) Text('${dayMin}m',
                        style: TextStyle(fontSize: 8, color: accent.withOpacity(0.5))),
                  ]),
                ),
              ),
            );
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ...days.map((d) {
              ...
              final dayLogs = widget.logs
                  .where((l) => l.endTime > ds && l.startTime < de).toList();
                ...
                child: SizedBox(
                  ...
                  child: Stack(children: [
                    ...
                    ...dayPoms.map((pom) {
                      ...
                      final h = ((re - rs) / 3600000 * hourH).clamp(3.0, 9999.0);
                      return Positioned(
                        top: top, left: 0, right: 0, height: h,
  ...
  Widget _buildTagSidebar(List<DateTime> days, int weekStart, int weekEnd) {
    ...
    int _tagWeekMin(PomodoroTag t) {
      final logMin = widget.logs
          .where((l) => l.tagUuids.contains(t.uuid) &&
          l.endTime > weekStart && l.startTime < weekEnd)
          .fold(0, (s, l) => s + (l.endTime - l.startTime) ~/ 60000);
      final pomMin = widget.pomodoros
          .where((p) => p.tagUuids.contains(t.uuid) &&
          p.startTime < weekEnd &&
          (p.endTime ?? p.startTime + p.effectiveDuration * 1000) > weekStart)
          .fold(0, (s, p) => s + p.effectiveDuration ~/ 60);
      return logMin + pomMin;
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Text('本周标签', style: TextStyle(
              fontSize: 10, color: _TC.textHint(context),
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(t.name, style: TextStyle(fontSize: 11, color: c,
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Text(t.name, style: TextStyle(fontSize: 9, color: c,
                    fontWeight: FontWeight.w700, letterSpacing: 0.8)),
                Text('${(tw / 60).toStringAsFixed(1)}h',
                    style: TextStyle(fontSize: 11, color: _TC.text(context),
                        fontWeight: FontWeight.w700)),
              ]),
            ),
          );
...
class _WeekDayColumn extends StatelessWidget {
  ...
  const _WeekDayColumn({
    required this.date, required this.logs, required this.pomodoros,
    required this.tags, required this.onTap, required this.onPomodoroTap,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final de = ds + 86400000;
    final dayLogs = logs.where((l) => l.endTime > ds && l.startTime < de).toList();
      ...
      child: SizedBox(
        ...
        child: Stack(children: [
          ...
          CustomPaint(
            ...
            painter: _WeekColumnPainter(dayLogs: dayLogs, tags: tags, dayStartMs: ds, isDark: isDark),
          ),
          ...
          ...dayPoms.map((pom) {
            ...
            return Positioned(
              top: top, right: 0, width: 14, height: h,
              child: GestureDetector(
...
class _WeekColumnPainter extends CustomPainter {
  ...
  _WeekColumnPainter({required this.dayLogs, required this.tags, required this.dayStartMs, required this.isDark});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final log in dayLogs) {
      ...
      if (log.tagUuids.isNotEmpty) {
        final t = tags.cast<PomodoroTag?>().firstWhere((t) => log.tagUuids.contains(t?.uuid), orElse: () => null);
        if (t != null) c = hexColor(t.color, opacity: 0.45);
      }
      canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTRB(0, top + 0.5, 3, bottom - 0.5), const Radius.circular(2)),
          Paint()..color = c.withOpacity(0.9));
      canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTRB(3, top + 0.5, size.width - 1, bottom - 0.5), const Radius.circular(3)),
          Paint()..color = c.withOpacity(0.28));
    }
  }

  @override
  bool shouldRepaint(covariant _WeekColumnPainter old) => old.isDark != isDark || old.dayLogs.length != dayLogs.length;
}
...
class _WeekColumnPainterV2 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      ...
      Paint()..color = isDark ? const Color(0xFF1A1A1A) : const Color(0xFFE5E5E5)..strokeWidth = 0.5,
    );
    ...
    for (final pom in dayPoms) {
      ...
      if (pom.tagUuids.isNotEmpty) {
        final t = tags.cast<PomodoroTag?>()
            .firstWhere((t) => pom.tagUuids.contains(t?.uuid), orElse: () => null);
        if (t != null) c = hexColor(t.color, opacity: 0.45);
      }

      // 左侧细色条
      canvas.drawRRect(
          RRect.fromRectAndRadius(Rect.fromLTRB(0, top + 0.5, 3, bottom - 0.5), const Radius.circular(2)),
          Paint()..color = c.withOpacity(0.9));
      // 主体填充
      canvas.drawRRect(
          RRect.fromRectAndRadius(Rect.fromLTRB(3, top + 0.5, size.width - 1, bottom - 0.5), const Radius.circular(3)),
    ...
    for (final log in dayLogs) {
      ...
      if (log.tagUuids.isNotEmpty) {
        final t = tags.cast<PomodoroTag?>()
            .firstWhere((t) => log.tagUuids.contains(t?.uuid), orElse: () => null);
        if (t != null) c = hexColor(t.color, opacity: 0.45);
      }
      canvas.drawRRect(
          RRect.fromRectAndRadius(Rect.fromLTRB(0, top + 0.5, 3, bottom - 0.5), const Radius.circular(2)),
  ...
  bool shouldRepaint(covariant _WeekColumnPainterV2 old) =>
      old.isDark != isDark || old.hourH != hourH ||
          old.dayLogs.length != dayLogs.length || old.dayPoms.length != dayPoms.length;
}
...
class _TimeAxis extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: kTimeColW, height: kTotalH,
      child: Stack(children: List.generate(24, (h) => Positioned(
        top: kVPad + h * kHourHeight - 8, right: 8,
        child: Text('${h.toString().padLeft(2, '0')}:00',
            style: TextStyle(fontSize: 9,
...
class _DayView extends StatefulWidget {
  ...
  const _DayView({
    required this.date, required this.crossDay, required this.logs, required this.pomodoros,
    required this.tags, required this.onBack, required this.onCrossDayChanged,
    required this.onSaveLog, required this.onPomodoroTap,
  });
...
class _DayViewState extends State<_DayView> {
  ...
  int get _ss => _dragStart != null && _dragEnd != null ? min(_dragStart!, _dragEnd!) : 0;
  ...
  void _openEntry() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _LogEntrySheet(
        initialStart: st, initialEnd: en, initialTagId: _selectedTagId, tags: widget.tags,
        onSave: (log) { Navigator.pop(context); widget.onSaveLog(log); },
      ),
    );
  }
  ...
  Widget _buildCrossDayBar() {
    return Container(
      ...
      child: Row(children: [
        ...
        Transform.scale(
          scale: 0.78, alignment: Alignment.centerLeft,
          child: Switch(
            value: widget.crossDay, onChanged: widget.onCrossDayChanged,
        ...
        if (widget.crossDay) ...[
          ...
          Text('${DateFormat('MM/dd').format(_gridStart)} → ${DateFormat('MM/dd').format(widget.date)}',
        ...
        ...([5, 10, 15, 30].map((m) => Padding(
          ...
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: _minutesPerBlock == m ? Theme.of(context).colorScheme.primary : _TC.btnBg(context),
                border: Border.all(color: _minutesPerBlock == m ? Theme.of(context).colorScheme.primary : _TC.btnBorder(context)),
  ...
  Widget _buildGrid(double hourH, double totalH) {
    return LayoutBuilder(builder: (ctx, constraints) {
      ...
      final isDark = Theme.of(ctx).brightness == Brightness.dark;
      final accent = Theme.of(ctx).colorScheme.primary;
      final selTag = widget.tags.cast<PomodoroTag?>().firstWhere((t) => t?.uuid == _selectedTagId, orElse: () => null);
    ...
    return Container(
      ...
      child: Column(children: [
        Padding(
          ...
          child: Text('TAGS', style: TextStyle(fontSize: 8, color: _TC.textHint(context), letterSpacing: 2)),
        ),
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: sel ? c : c.withOpacity(0.2), width: sel ? 1.5 : 1),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Column(children: [
                  Container(width: 7, height: 7, decoration: BoxDecoration(color: c, shape: BoxShape.circle,
                      boxShadow: sel ? [BoxShadow(color: c, blurRadius: 5)] : [])),
                  const SizedBox(height: 4),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 4),
                    child: Text(t.name, textAlign: TextAlign.center,
                        style: TextStyle(fontSize: 9, color: sel ? c : _TC.textSub(ctx),
    ...
    return Container(
      ...
      child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
          Text('$_durMin 分钟', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _TC.text(context))),
          Text('${DateFormat('HH:mm').format(st)} → ${DateFormat('HH:mm').format(en)}',
              style: TextStyle(fontSize: 11, color: _TC.textSub(context))),
        ])),
        TextButton(
          onPressed: () => setState(() { _dragStart = null; _dragEnd = null; }),
        ...
        ElevatedButton(
          ...
          child: const Text('详情录入', style: TextStyle(fontWeight: FontWeight.w700)),
        ),
      ]),
    );
  }
...
class _DayGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final mj = Paint()..color = isDark ? const Color(0xFF1E1E1E) : const Color(0xFFDDDDDD)..strokeWidth = 0.5;
    ...
    for (int c = 0; c <= bpr; c++) {
      canvas.drawLine(Offset(c * bw, 0), Offset(c * bw, size.height), c % 4 == 0 ? mj : mn);
    }
    ...
    for (final log in logs) {
      ...
      final eIdx = ((eMs - gsMs - 1) ~/ (60000 * minutesPerBlock)).clamp(0, 24 * bpr - 1);
      PomodoroTag? tag;
      if (log.tagUuids.isNotEmpty) {
        tag = tags.cast<PomodoroTag?>().firstWhere((t) => log.tagUuids.contains(t?.uuid), orElse: () => null);
      }
      final fill = tag != null ? hexColor(tag.color, opacity: isDark ? 0.14 : 0.1)
          : (isDark ? const Color(0x18888888) : const Color(0x0F000000));
      final bar = tag != null ? hexColor(tag.color, opacity: 0.5)
...
class _LogEntrySheetState extends State<_LogEntrySheet> {
  ...
  void initState() {
    super.initState();
    _start = widget.initialStart; _end = widget.initialEnd; _tagId = widget.initialTagId;
    final tag = widget.tags.cast<PomodoroTag?>().firstWhere((t) => t?.uuid == _tagId, orElse: () => null);
  ...
  Future<void> _pickTime(bool isStart) async {
    ...
    if (t == null) return;
    setState(() {
      if (isStart) _start = DateTime(_start.year, _start.month, _start.day, t.hour, t.minute);
      else _end = DateTime(_end.year, _end.month, _end.day, t.hour, t.minute);
    });
  }
  ...
  Future<void> _pickDate(bool isStart) async {
    ...
    if (d == null) return;
    setState(() {
      if (isStart) _start = DateTime(d.year, d.month, d.day, _start.hour, _start.minute);
      else _end = DateTime(d.year, d.month, d.day, _end.hour, _end.minute);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('补录事件', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: _TC.text(context))),
          const Spacer(),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
            decoration: BoxDecoration(
            ...
            child: Text(_durLabel(), style: const TextStyle(fontSize: 12, color: Color(0xFF4ADE80), fontWeight: FontWeight.w600)),
          ),
        ]),
        const SizedBox(height: 16),

        Text('CATEGORY', style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 2)),
        const SizedBox(height: 8),
        Wrap(spacing: 8, runSpacing: 6, children: widget.tags.map((t) {
          final c = hexColor(t.color);
          final sel = _tagId == t.uuid;
          return GestureDetector(
            onTap: () => setState(() { _tagId = t.uuid; _tc.text = t.name; }),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 150),
              ...
              child: Text(t.name, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
        ...
        Text('TITLE', style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 2)),
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            hintStyle: TextStyle(color: _TC.textHint(context), fontSize: 13),
            filled: true, fillColor: _TC.inputFill(context),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: _TC.divider(context))),
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: _TC.divider(context))),
        ...
        Row(children: [
          Expanded(child: _TimePickerCard(label: '开始时间', dt: _start, onTapTime: () => _pickTime(true), onTapDate: () => _pickDate(true))),
          const SizedBox(width: 10),
          Expanded(child: _TimePickerCard(label: '结束时间', dt: _end, onTapTime: () => _pickTime(false), onTapDate: () => _pickDate(false))),
        ]),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: const Text('保存记录', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
          ),
        ),
      ]),
    );
...
class _TimePickerCard extends StatelessWidget {
  ...
  const _TimePickerCard({required this.label, required this.dt, required this.onTapTime, required this.onTapDate});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: _TC.inputFill(context), border: Border.all(color: _TC.divider(context)), borderRadius: BorderRadius.circular(12)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 1)),
          const SizedBox(height: 4),
          GestureDetector(
            onTap: onTapDate,
            child: Text(DateFormat('MM月dd日').format(dt), style: TextStyle(fontSize: 11, color: _TC.textSub(context))),
          ),
          const SizedBox(height: 4),
          Row(children: [
            Text(DateFormat('HH:mm').format(dt), style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: _TC.text(context))),
...
class _TagDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxMin = allRecords.isEmpty ? 0 : allRecords.map((r) => r.durationMin).reduce(max);
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 12, height: 12, decoration: BoxDecoration(color: c, shape: BoxShape.circle, boxShadow: [BoxShadow(color: c, blurRadius: 8)])),
          const SizedBox(width: 10),
          Text(tag.name, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: _TC.text(context))),
          const Spacer(),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
            decoration: BoxDecoration(color: c.withOpacity(0.12), border: Border.all(color: c.withOpacity(0.3)), borderRadius: BorderRadius.circular(20)),
            child: Text('${(totalMin / 60).toStringAsFixed(1)}h 累计', style: TextStyle(fontSize: 11, color: c, fontWeight: FontWeight.w600)),
          ),
          IconButton(icon: Icon(Icons.close, color: _TC.textHint(context), size: 20), onPressed: () => Navigator.pop(context)),
        ]),
        const SizedBox(height: 16),

        Row(children: [
          _StatCard(label: '记录次数', value: '${allRecords.length}次', color: c),
          const SizedBox(width: 10),
        ...
        Text('DURATION TREND', style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 2)),
        const SizedBox(height: 8),
        if (chartData.length >= 2)
          _MiniLineChart(data: chartData.map((r) => r.durationMin).toList(), color: c, height: 80)
        else
          Text('数据采集中...', style: TextStyle(fontSize: 11, color: _TC.textHint(context))),
        const SizedBox(height: 16),

        Text('START TIME TREND', style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 2)),
        const SizedBox(height: 8),
        if (chartData.length >= 2)
          _MiniLineChart(
            data: chartData.map((r) { final d = DateTime.fromMillisecondsSinceEpoch(r.startTime); return d.hour * 60 + d.minute; }).toList(),
            color: _TC.textSub(context), height: 55,
          ),
        const SizedBox(height: 16),

        Text('RECENT RECORDS', style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 2)),
        const SizedBox(height: 8),
        ConstrainedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (ctx, i) {
              ...
              final startDt = DateTime.fromMillisecondsSinceEpoch(r.startTime);
              final endDt = DateTime.fromMillisecondsSinceEpoch(r.endTime);
              return Container(
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                decoration: BoxDecoration(
                  color: _TC.inputFill(ctx),
                  border: Border(left: BorderSide(color: r.isPomodoro ? Colors.redAccent : c, width: 3)),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Row(children: [
                  if (r.isPomodoro) ...[
                    Icon(Icons.timer_outlined, size: 12, color: Colors.redAccent.withOpacity(0.6)),
                    const SizedBox(width: 4),
                  ],
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(r.title, style: TextStyle(fontSize: 12, color: _TC.text(ctx), fontWeight: FontWeight.w600)),
                    const SizedBox(height: 2),
                    Text('${DateFormat('MM/dd').format(startDt)} ${DateFormat('HH:mm').format(startDt)} → ${DateFormat('HH:mm').format(endDt)}',
                        style: TextStyle(fontSize: 10, color: _TC.textHint(ctx))),
                  ])),
                  Text('${r.durationMin}min', style: TextStyle(fontSize: 13,
                      color: r.isPomodoro ? Colors.redAccent : c, fontWeight: FontWeight.w700)),
                  // 只有 TimeLogItem 才能删除（番茄钟不在这里管理）
                  if (!r.isPomodoro)
                    IconButton(
                      icon: Icon(Icons.delete_outline, color: Colors.red.withOpacity(0.55), size: 18),
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in pts) {
      canvas.drawCircle(p, 4, Paint()..color = color.withOpacity(0.15)..style = PaintingStyle.stroke..strokeWidth = 2);
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Expanded(child: Container(
      ...
      decoration: BoxDecoration(color: _TC.inputFill(context), borderRadius: BorderRadius.circular(10)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 10, color: _TC.textHint(context))),
        const SizedBox(height: 4),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 1)),
        const SizedBox(height: 4),
        Text(value, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: _TC.text(context))),
...
class _TagManagerSheetState extends State<_TagManagerSheet> {
  ...
  void initState() {
    super.initState();
    _list = widget.tags.map((t) => PomodoroTag(uuid: t.uuid, name: t.name, color: t.color)).toList();
  }
  ...
  void _add() {
    ...
    setState(() {
      _list.add(PomodoroTag(uuid: 'tag_${DateTime.now().millisecondsSinceEpoch}', name: _nc.text.trim(), color: _newColor));
      _nc.clear();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('标签管理', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _TC.text(context))),
          const Spacer(),
          IconButton(icon: Icon(Icons.close, color: _TC.textHint(context), size: 20), onPressed: () => Navigator.pop(context)),
        ]),
            ...
            itemBuilder: (ctx, i) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: TextField(
                      controller: TextEditingController(text: t.name)..selection = TextSelection.collapsed(offset: t.name.length),
                      onChanged: (v) => _list[i] = PomodoroTag(uuid: t.uuid, name: v, color: t.color),
                      style: TextStyle(fontSize: 12, color: _TC.text(ctx)),
                      decoration: InputDecoration(
                        isDense: true, filled: true, fillColor: _TC.inputFill(ctx),
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: _TC.divider(ctx))),
                        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: _TC.divider(ctx))),
                        contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
                      ),
                    ),
                  ),
                  const SizedBox(width: 6),
                  Wrap(spacing: 3, children: kPalette.map((pc) => GestureDetector(
                    onTap: () => setState(() => _list[i] = PomodoroTag(uuid: t.uuid, name: t.name, color: pc)),
                    child: Container(width: 14, height: 14,
                        decoration: BoxDecoration(color: hexColor(pc), shape: BoxShape.circle,
                            border: Border.all(color: t.color == pc ? Colors.white : Colors.transparent, width: 1.5))),
                  )).toList()),
                  const SizedBox(width: 6),
                  GestureDetector(
                    onTap: () => setState(() => _list.removeAt(i)),
                    child: Icon(Icons.delete_outline, color: Colors.red.withOpacity(0.55), size: 18),
                  ),
                ]),
              );
            },
        ...
        Text('NEW TAG', style: TextStyle(fontSize: 9, color: _TC.textHint(context), letterSpacing: 2)),
        const SizedBox(height: 8),
        Row(children: [
          Expanded(
            child: TextField(
              controller: _nc, onSubmitted: (_) => _add(),
              style: TextStyle(fontSize: 13, color: _TC.text(context)),
              decoration: InputDecoration(
                hintText: '标签名称', hintStyle: TextStyle(color: _TC.textHint(context)),
                isDense: true, filled: true, fillColor: _TC.inputFill(context),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: _TC.divider(context))),
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: _TC.divider(context))),
          ...
          ElevatedButton(
            onPressed: _add,
            style: ElevatedButton.styleFrom(
              backgroundColor: accent.withOpacity(0.15), foregroundColor: accent,
        ...
        const SizedBox(height: 8),
        Wrap(spacing: 6, children: kPalette.map((pc) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(color: cc, shape: BoxShape.circle,
                  border: Border.all(color: _newColor == pc ? Colors.white : Colors.transparent, width: 2),
                  boxShadow: _newColor == pc ? [BoxShadow(color: cc, blurRadius: 6)] : []),
            ),
          );
        }).toList()),
        const SizedBox(height: 16),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: primary ? Colors.transparent : _TC.btnBorder(context)),
          borderRadius: BorderRadius.circular(8),
        ),
        child: Text(label, style: TextStyle(
          fontSize: 11, fontWeight: primary ? FontWeight.w700 : FontWeight.w400,
