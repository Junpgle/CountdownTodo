...
class _TC {
  ...
  static Color card(BuildContext c)      => Theme.of(c).brightness == Brightness.dark
      ? const Color(0xFF1A1A1A) : Theme.of(c).colorScheme.surfaceVariant;
  static Color topBar(BuildContext c)    => Theme.of(c).brightness == Brightness.dark
      ? const Color(0xFF0D0D0D) : Theme.of(c).colorScheme.surface;
  static Color inputFill(BuildContext c) => Theme.of(c).brightness == Brightness.dark
      ? const Color(0xFF181818) : Theme.of(c).colorScheme.surfaceVariant;
  static Color text(BuildContext c)      => Theme.of(c).colorScheme.onSurface;
  static Color textSub(BuildContext c)   => Theme.of(c).colorScheme.onSurface.withOpacity(0.55);
  static Color textHint(BuildContext c)  => Theme.of(c).colorScheme.onSurface.withOpacity(0.28);
  static Color divider(BuildContext c)   => Theme.of(c).dividerColor;
  static Color btnBg(BuildContext c)     => Theme.of(c).brightness == Brightness.dark
      ? const Color(0xFF222222) : Theme.of(c).colorScheme.surfaceVariant;
  static Color btnBorder(BuildContext c) => Theme.of(c).brightness == Brightness.dark
      ? const Color(0xFF333333) : Theme.of(c).dividerColor;
  static Color timeLabel(BuildContext c, {bool major = false}) => major
      ? (Theme.of(c).brightness == Brightness.dark ? const Color(0xFF888888) : const Color(0xFF666666))
      : (Theme.of(c).brightness == Brightness.dark ? const Color(0xFF444444) : const Color(0xFFAAAAAA));
}
...
class _TimeLogScreenState extends State<TimeLogScreen> {
  ...
  void _goDay(DateTime d, {_DayMode mode = _DayMode.view}) =>
      setState(() { _focusedDate = d; _view = _ViewMode.day; _dayMode = mode; });
  ...
  Widget _buildTitle() {
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      GestureDetector(
        onTap: () async {
          final p = await showDatePicker(context: context,
              initialDate: _focusedDate, firstDate: DateTime(2020),
      ...
      IconButton(icon: const Icon(Icons.chevron_right, size: 20),
          padding: EdgeInsets.zero, constraints: const BoxConstraints(),
          onPressed: () => setState(() => _focusedDate =
              _focusedDate.add(const Duration(days: 1)))),
    ]);
  }
  ...
  List<Widget> _buildActions() {
    ...
    if (_view == _ViewMode.day) {
      acts.add(TextButton(
        onPressed: () => setState(() =>
        _dayMode = _dayMode == _DayMode.view ? _DayMode.edit : _DayMode.view),
      ...
      if (_dayMode == _DayMode.edit)
        acts.add(IconButton(
            ...
            onPressed: _showTagManager, tooltip: '标签管理'));
    }
    acts.add(IconButton(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if (_view != _ViewMode.week) { _goWeek(); return false; }
        return true;
      },
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          bottom: PreferredSize(
            ...
            child: Container(height: 34, color: _TC.topBar(context),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                _ViewTab(label: '周', selected: _view == _ViewMode.week,
                    onTap: () { if (_view != _ViewMode.week) _goWeek(); }),
                const SizedBox(width: 6),
                _ViewTab(label: '日', selected: _view == _ViewMode.day,
                    onTap: () { if (_view != _ViewMode.day)
        ...
        body: _isLoading
            ...
            ? _WeekView(
            weekStart: _weekStart, logs: _allLogs,
            pomodoros: _allPomodoros, tags: _tags,
            onDayTap: (d) => _goDay(d),
            onTagTap: _showTagDetail, onManageTags: _showTagManager,
            onAddLog: () => _goDay(DateTime.now(), mode: _DayMode.edit),
            onPomodoroTap: _showPomodoroDetail,
            onTimeLogTap: _showTimeLogDetail, username: widget.username)
            : _dayMode == _DayMode.view
            ? _DayGridView(
            date: _focusedDate, logs: _allLogs,
            pomodoros: _allPomodoros, tags: _tags,
            onPomodoroTap: _showPomodoroDetail,
            onTimeLogTap: _showTimeLogDetail,
            onSwitchEdit: () => setState(() => _dayMode = _DayMode.edit))
            : _DayView(
            date: _focusedDate, crossDay: _crossDay,
            logs: _allLogs, pomodoros: _allPomodoros, tags: _tags,
            onBack: _goWeek,
  ...
  void _showPomodoroDetail(PomodoroRecord pom) {
    final tag   = pom.tagUuids.isNotEmpty ? _tags.cast<PomodoroTag?>().firstWhere(
            (t) => t?.uuid == pom.tagUuids.first, orElse: () => null) : null;
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent,
        builder: (_) => _sheet(Colors.redAccent.withOpacity(0.4),
          Column(mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  _glow(Colors.redAccent),  const SizedBox(width: 10),
                  Text('专注记录', style: TextStyle(fontSize: 18,
                      fontWeight: FontWeight.w700, color: _TC.text(context))),
                  const Spacer(), _pill('${dur}min', Colors.redAccent),
                  IconButton(icon: Icon(Icons.close, size: 18, color: _TC.textHint(context)),
                      onPressed: () => Navigator.pop(context)),
                ]),
                if (tag != null) ...[const SizedBox(height: 10), _tagRow(tag.name, tc)],
                const SizedBox(height: 16),
                Row(children: [
                  Expanded(child: _InfoCard(label: '开始',
                      value: DateFormat('HH:mm').format(s), sub: DateFormat('MM/dd').format(s))),
                  const SizedBox(width: 10),
                  Expanded(child: _InfoCard(label: '结束',
                      value: DateFormat('HH:mm').format(e), sub: DateFormat('MM/dd').format(e))),
                  const SizedBox(width: 10),
                  Expanded(child: _InfoCard(label: '时长', value: '${dur}min',
  ...
  void _showTimeLogDetail(TimeLogItem log) {
    final tag = log.tagUuids.isNotEmpty ? _tags.cast<PomodoroTag?>().firstWhere(
            (t) => t?.uuid == log.tagUuids.first, orElse: () => null) : null;
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent,
        builder: (_) => _sheet(c.withOpacity(0.4),
          Column(mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  _glow(c), const SizedBox(width: 10),
                  Expanded(child: Text(
                      log.title.isNotEmpty ? log.title : (tag?.name ?? '补录记录'),
                ...
                if (tag != null) ...[const SizedBox(height: 10), _tagRow(tag.name, c)],
                const SizedBox(height: 16),
                Row(children: [
                  Expanded(child: _InfoCard(label: '开始',
                      value: DateFormat('HH:mm').format(s), sub: DateFormat('MM/dd').format(s))),
  ...
  Widget _glow(Color c) => Container(width: 10, height: 10,
  ...
  Widget _tagRow(String name, Color c) => Row(children: [
    ...
    Text(name, style: TextStyle(fontSize: 13, color: c, fontWeight: FontWeight.w600)),
  ]);

  void _showTagDetail(PomodoroTag tag) {
    showModalBottomSheet(context: context, isScrollControlled: true,
        backgroundColor: Colors.transparent,
        builder: (_) => _TagDetailSheet(tag: tag, logs: _allLogs,
...
class _WeekView extends StatelessWidget {
  ...
  final List<TimeLogItem> logs; final List<PomodoroRecord> pomodoros;
  ...
  const _WeekView({
    required this.weekStart, required this.logs, required this.pomodoros,
    required this.tags, required this.onDayTap, required this.onTagTap,
    required this.onManageTags, required this.onAddLog,
    required this.onPomodoroTap, required this.onTimeLogTap, required this.username});

  int _rangeMin(int from, int to) {
    final lm = logs.where((l) => l.endTime > from && l.startTime < to)
        .fold(0, (s, l) => s + (l.endTime - l.startTime) ~/ 60000);
    final pm = pomodoros.where((p) => p.startTime < to &&
    ...
    return LayoutBuilder(builder: (ctx, outer) {
      ...
      return Column(children: [
        ...
        Expanded(child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(child: _buildGrid(ctx, days, outer.maxWidth - (isWide ? 160.0 : 0))),
  ...
  Widget _buildTopBar(BuildContext ctx, int todayMin) => Container(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 8),
      Builder(builder: (bCtx) => ElevatedButton(
        style: ElevatedButton.styleFrom(backgroundColor: Colors.green),
        onPressed: () async {
          await StorageService.resetSyncTime(username);
          await StorageService.syncData(username, forceFullSync: true,
              syncTodos: true, syncCountdowns: true, syncTimeLogs: true);
          if (bCtx.mounted) ScaffoldMessenger.of(bCtx).showSnackBar(const SnackBar(
              content: Text('🎉 数据强拉成功！请点击右上角的【刷新图标 ↻】查看界面'),
              duration: Duration(seconds: 4)));
        },
        child: const Text('🚑 一键恢复', style: TextStyle(color: Colors.white, fontSize: 12)),
      )),
    ]),
  );
  ...
  Widget _buildGrid(BuildContext ctx, List<DateTime> days, double availW) {
    ...
    return Column(children: [
      Container(height: 52, color: _TC.topBar(ctx),
          child: Row(children: [
            SizedBox(width: tW),
            ...List.generate(7, (i) {
              final d       = days[i];
              ...
              final dm = dayMin(d);
              return GestureDetector(onTap: () => onDayTap(d),
                  child: Container(width: dW, height: 52,
      ...
      Expanded(child: LayoutBuilder(builder: (ctx2, gc) {
        ...
        return SizedBox(height: totalH,
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              SizedBox(width: tW, height: totalH,
                  child: Stack(clipBehavior: Clip.none,
                      children: List.generate(24, (h) => Positioned(
                          top: h * hourH - 8, right: 4,
              ...
              ...days.map((d) {
                ...
                final dLogs = logs.where((l) => l.endTime > ds && l.startTime < de).toList();
                final dPoms = pomodoros.where((p) {
                  final pe = p.endTime ?? (p.startTime + p.effectiveDuration * 1000);
                  return pe > ds && p.startTime < de;
                }).toList();
                return SizedBox(width: dW, height: totalH, child: Stack(children: [
                  CustomPaint(size: Size(dW, totalH),
                      painter: _WeekColPainter(dayLogs: dLogs, dayPoms: dPoms,
                          tags: tags, dayStartMs: ds, isDark: isDark, hourH: hourH)),
                  ...dPoms.map((pom) {
                    final pe  = pom.endTime ?? (pom.startTime + pom.effectiveDuration * 1000);
                    final rs  = max(pom.startTime, ds); final re = min(pe, de);
                    final top = (rs - ds) / 3600000 * hourH;
                    final h   = ((re - rs) / 3600000 * hourH).clamp(3.0, 9999.0);
                    return Positioned(top: top, left: 0, right: 0, height: h,
                        child: GestureDetector(onTap: () => onPomodoroTap(pom),
                            behavior: HitTestBehavior.opaque,
                            child: const SizedBox.expand()));
                  }),
                  ...dLogs.map((log) {
                    final rs  = max(log.startTime, ds); final re = min(log.endTime, de);
                    final top = (rs - ds) / 3600000 * hourH;
                    final h   = ((re - rs) / 3600000 * hourH).clamp(3.0, 9999.0);
                    return Positioned(top: top, left: 0, right: 0, height: h,
                        child: GestureDetector(onTap: () => onTimeLogTap(log),
                            behavior: HitTestBehavior.opaque,
                            child: const SizedBox.expand()));
                  }),
                ]));
  ...
  Widget _buildTagSidebar(BuildContext ctx, int ws, int we) => Container(
      width: 160,
      decoration: BoxDecoration(color: _TC.card(ctx),
          border: Border(left: BorderSide(color: _TC.divider(ctx)))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.fromLTRB(12, 14, 12, 8),
            child: Text('本周标签', style: TextStyle(fontSize: 10,
                color: _TC.textHint(ctx), fontWeight: FontWeight.w700, letterSpacing: 1.5))),
        Expanded(child: ListView(padding: const EdgeInsets.only(bottom: 8),
            children: tags.map((t) {
              int tw = logs.where((l) => l.tagUuids.contains(t.uuid)
                  && l.endTime > ws && l.startTime < we)
                  .fold(0, (s, l) => s + (l.endTime - l.startTime) ~/ 60000);
              tw += pomodoros.where((p) => p.tagUuids.contains(t.uuid)
                  && p.startTime < we
                  && (p.endTime ?? p.startTime + p.effectiveDuration * 1000) > ws)
                  .fold(0, (s, p) => s + p.effectiveDuration ~/ 60);
              final c = hexColor(t.color);
              return GestureDetector(onTap: () => onTagTap(t),
                  child: Container(
                        ...
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              ...
                              Text('${(tw / 60).toStringAsFixed(1)}h',
                                  style: TextStyle(fontSize: 13, color: _TC.text(ctx),
                                      fontWeight: FontWeight.w700)),
                            ])),
                      ])));
            }).toList())),
  ...
  Widget _buildTagChips(BuildContext ctx, int ws, int we) => Container(
      height: 52, color: _TC.topBar(ctx),
            ...
            return GestureDetector(onTap: () => onTagTap(t),
                child: Container(
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      Text('${(tw / 60).toStringAsFixed(1)}h',
                          style: TextStyle(fontSize: 11, color: _TC.text(ctx),
                              fontWeight: FontWeight.w700)),
                    ])));
          }).toList()));
}
...
class _WeekColPainter extends CustomPainter {
  final List<TimeLogItem> dayLogs; final List<PomodoroRecord> dayPoms;
  final List<PomodoroTag> tags; final int dayStartMs;
  final bool isDark; final double hourH;

  const _WeekColPainter({required this.dayLogs, this.dayPoms = const [],
    required this.tags, required this.dayStartMs,
    required this.isDark, required this.hourH});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(size.width - 0.5, 0), Offset(size.width - 0.5, size.height),
        Paint()..color = isDark ? const Color(0xFF1A1A1A) : const Color(0xFFE5E5E5)
          ..strokeWidth = 0.5);

    void block(int start, int end, Color c, bool isPom) {
      final rs  = max(start, dayStartMs); final re = min(end, de);
      final top = (rs - dayStartMs) / 3600000 * hourH;
      final bot = (re - dayStartMs) / 3600000 * hourH;
      if (bot <= top + 1) return;
      canvas.drawRRect(RRect.fromRectAndRadius(
          Rect.fromLTRB(0, top + 0.5, 3, bot - 0.5), const Radius.circular(2)),
    ...
    for (final p in dayPoms) {
      ...
      if (p.tagUuids.isNotEmpty) {
        final t = tags.cast<PomodoroTag?>().firstWhere(
                (t) => p.tagUuids.contains(t?.uuid), orElse: () => null);
    ...
    for (final l in dayLogs) {
      ...
      if (l.tagUuids.isNotEmpty) {
        final t = tags.cast<PomodoroTag?>().firstWhere(
                (t) => l.tagUuids.contains(t?.uuid), orElse: () => null);
        if (t != null) c = hexColor(t.color, opacity: 0.45);
      }
  ...
  @override bool shouldRepaint(covariant _WeekColPainter o) =>
      o.isDark != isDark || o.hourH != hourH ||
          o.dayLogs.length != dayLogs.length || o.dayPoms.length != dayPoms.length;
}
...
//  坐标系：
//    行 = 小时 (0-23)          → Y轴，纵向滚动
//    列 = 每行内5分钟格(0-11)   → X轴，横向固定（屏宽内平分）
//
//  事件绘制规则：
//    · startMin = 相对当天的分钟数
//    · row  = startMin / 60          → 起始行
//    · col  = (startMin % 60) / 5    → 行内起始列
//    · 事件持续跨行时，逐行渲染片段
//    · 宽 = 本行内占用的列数 * colW
//    · 高 = rowH（始终填满整行高）
// ══════════════════════════════════════════════════════════
class _DayGridView extends StatefulWidget {
  ...
  const _DayGridView({
    ...
    required this.tags, required this.onPomodoroTap,
    required this.onTimeLogTap, required this.onSwitchEdit});

  @override State<_DayGridView> createState() => _DayGridViewState();
}
...
class _DayGridViewState extends State<_DayGridView> {
  ...
  Widget build(BuildContext context) {
    ...
    final logMin   = dLogs.fold(0, (s, l) => s + (l.endTime - l.startTime) ~/ 60000);
    ...
    for (final l in dLogs)
      for (final uid in l.tagUuids)
        tagMinMap[uid] = (tagMinMap[uid] ?? 0) + (l.endTime - l.startTime) ~/ 60000;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          GestureDetector(onTap: widget.onSwitchEdit,
              child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const SizedBox(width: 4),
                    Text('补录', style: TextStyle(fontSize: 11, color: accent,
                        fontWeight: FontWeight.w700)),
                  ]))),
        ]),
        if (tagMinMap.isNotEmpty && total > 0) ...[
          const SizedBox(height: 8),
          _TagProgressBar(tagMinMap: tagMinMap, totalMin: total, tags: widget.tags),
        ],
      ]),
    );
  }
              ...
              child: Align(
                ...
                child: Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 9,
                      fontWeight: h % 6 == 0 ? FontWeight.w700 : FontWeight.w400,
    ...
    return Stack(children: [
      ...
      ...dPoms.expand((pom) {
        ...
        return _buildEventSegments(
          ...
          dayStartMs: dayStartMs, colW: colW, isDark: isDark,
          fillColor: base.withOpacity(isDark ? 0.30 : 0.22),
          barColor:  base,
          isPom: true, title: title,
              ...
              children: [
                if (isPom && showIcon)
                  Padding(
                    ...
                    child: Icon(Icons.timer_outlined, size: 9, color: barColor),
                  ),
                Expanded(
                  child: Text(
                    title,
...
class _TagProgressBar extends StatelessWidget {
  final Map<String, int> tagMinMap; final int totalMin;
  final List<PomodoroTag> tags;
  const _TagProgressBar({required this.tagMinMap, required this.totalMin, required this.tags});

  @override
  Widget build(BuildContext context) {
    final sorted = tagMinMap.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ClipRRect(borderRadius: BorderRadius.circular(4),
          child: SizedBox(height: 5, width: double.infinity,
              child: Row(children: sorted.map((e) {
                final t = tags.cast<PomodoroTag?>()
                    .firstWhere((t) => t?.uuid == e.key, orElse: () => null);
                final c = t != null ? hexColor(t.color) : const Color(0xFF3B82F6);
                return Expanded(flex: e.value, child: Container(color: c));
              }).toList()))),
      const SizedBox(height: 5),
      Wrap(spacing: 10, runSpacing: 3, children: sorted.map((e) {
        final t = tags.cast<PomodoroTag?>()
            .firstWhere((t) => t?.uuid == e.key, orElse: () => null);
        ...
        return Row(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 6, height: 6,
              decoration: BoxDecoration(color: c, shape: BoxShape.circle)),
          const SizedBox(width: 4),
          Text(t.name, style: TextStyle(fontSize: 10, color: c,
              fontWeight: FontWeight.w600)),
          const SizedBox(width: 3),
          Text('${e.value}m', style: TextStyle(fontSize: 10,
...
class _WeekDayColumn extends StatelessWidget {
  final DateTime date; final List<TimeLogItem> logs;
  final List<PomodoroRecord> pomodoros; final List<PomodoroTag> tags;
  final VoidCallback onTap; final void Function(PomodoroRecord) onPomodoroTap;
  const _WeekDayColumn({required this.date, required this.logs,
    required this.pomodoros, required this.tags, required this.onTap,
