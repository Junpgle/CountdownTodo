...
import '../../../services/island_action_channel.dart';
...
class PomodoroWorkbenchState extends State<PomodoroWorkbench>
  ...
  void _listenToIslandActions() {
    _islandSub = IslandActionChannel.actionStream.listen((actionData) {
      if (!mounted) return;
      if (actionData.isNotEmpty) {
        final action = actionData['action']?.toString();
        if (action == 'finish') {
          WindowFocusService.showAndFocus();
          _onFocusEnd();
    ...
    try {
      final results = await Future.wait([
        ...
        StorageService.getTodos(widget.username)
            .timeout(const Duration(seconds: 5), onTimeout: () => <TodoItem>[]),
        StorageService.getTodoGroups(widget.username).timeout(
            const Duration(seconds: 5),
            onTimeout: () => <TodoGroup>[]),
        PomodoroService.loadRunState()
    ...
    switch (signal.action) {
      ...
      case 'RECONNECT_SYNC':
        ...
        if (signal.serverElapsedMs != null) {
          ...
          if (isCountUp) {
            ...
            final plannedSecs =
                signal.duration ?? (_settings.focusMinutes * 60);
          ...
          if (isCountUp) {
            final referenceMs =
                isPaused ? (signal.pausedAtMs ?? timestamp) : nowMs;
      ...
      case 'SWITCH':
        if (_phase != PomodoroPhase.remoteWatching) break;
        {
          ...
          setState(() {
            _boundTodo =
                (signal.todoTitle != null && signal.todoTitle!.isNotEmpty)
                    ? TodoItem(
                        id: signal.todoUuid ?? '',
                        title: signal.todoTitle!,
                        isDone: false,
  ...
  void _pauseFocus() {
    ...
    _pauseIntervals.add(PauseInterval(startMs: _pausedAtMs));
  ...
  Future<void> _startFocus() async {
    ...
    _suppressRunStateEvents = true;
    final PomodoroStartResult startResult;
    try {
      startResult = await PomodoroControlService.startFocus(
        settings: _settings,
  ...
  Future<void> _switchTask(TodoItem? newTodo) async {
    ...
    if (actualSeconds > 5) {
      ...
      final pauseSecs = (_accumulatedMs / 1000).round();
      final intervals = List<PauseInterval>.from(_pauseIntervals);
  ...
  Future<bool> _askCompletionAndRecord(
    ...
    final record = PomodoroRecord.fromRunState(
      state: _buildCurrentRunState(),
      status: PomodoroRecordStatus.completed,
      endMs: isCountUp ? startMs + editedDuration * 1000 : endMs,
  ...
  PomodoroRunState _buildCurrentRunState() => PomodoroRunState(
        ...
        currentCycle: _currentCycle,
        totalCycles: _settings.cycles,
        focusSeconds: _settings.mode == TimerMode.countUp
            ? 0
            : _settings.focusMinutes * 60,
          ...
          : AnimatedOpacity(
              ...
              child: OrientationBuilder(
                builder: (context, orientation) {
                  ...
                  return Padding(
                    padding:
                        EdgeInsets.symmetric(horizontal: isLandscape ? 32 : 24),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.center,
                      children: [
                        _buildHeader(
                            isIdle, isFocusing, isRemoteWatching, isLandscape),
                        Expanded(
                        ...
                        if (!isLandscape)
                          const SafeArea(
                              top: false, child: SizedBox(height: 8)),
                      ],
                    ),
                  );
                },
  ...
  Widget _buildHeader(
      bool isIdle, bool isFocusing, bool isRemoteWatching, bool isLandscape) {
    final isGlobalLandscape =
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          AnimatedOpacity(
            opacity: (isFocusing || isRemoteWatching || isGlobalLandscape)
                ? 1.0
                : 0.0,
  ...
  Widget _buildTagsList(bool isRemoteWatching) {
    ...
    return Wrap(
        ...
        children: activeTags
            .map((t) => _SimpleTag(
                name: t.name, color: AppColorUtils.parseHex(t.color)))
            .toList());
  }
