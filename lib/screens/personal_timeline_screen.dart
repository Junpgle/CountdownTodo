...
class _PersonalTimelineScreenState extends State<PersonalTimelineScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final summary = await TimelineService.instance.getSummaryForRange(widget.username, start, end);
      
      // 2. Get detailed events for Daily only (too many for others)
      if (_dimension == TimelineDimension.daily) {
        _events = await TimelineService.instance.getEventsForDay(widget.username, day);
      } else {
      ...
      while (cursor.isBefore(end)) {
        ...
        for (var item in stats) {
          ...
          catUsage[cat] = (catUsage[cat] ?? 0) + (item['duration'] as int? ?? 0);
        }
      ...
      if (catUsage.isNotEmpty) {
        topCat = catUsage.entries.reduce((a, b) => a.value > b.value ? a : b).key;
      }
  ...
  Widget _buildBackground(ColorScheme cs) {
    return Container(
      ...
      child: Stack(
        children: [
          Positioned(bottom: 100, left: -100, child: _buildBlob(cs.secondary.withValues(alpha: 0.03), 400)),
        ],
      ),
    );
  }
  ...
  Widget _buildBlob(Color color, double size) {
    return Container(
      ...
      child: BackdropFilter(filter: ui.ImageFilter.blur(sigmaX: 80, sigmaY: 80), child: Container(color: Colors.transparent)),
    );
  }
  ...
  Widget _buildGreeting(ColorScheme cs) {
    ...
    switch (_dimension) {
      ...
      case TimelineDimension.weekly:
        ...
        label = '${DateFormat('MM/dd').format(start)} - ${DateFormat('MM/dd').format(end)}';
        sub = '本周回顾';
        break;
      case TimelineDimension.monthly:
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(sub, style: TextStyle(fontSize: 14, color: cs.primary, fontWeight: FontWeight.w600, letterSpacing: 1.2)),
                const SizedBox(height: 4),
                Text(label, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, fontFamily: 'Serif')),
                const SizedBox(height: 8),
                Text(
                  _getMotivationText(),
                  style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant.withValues(alpha: 0.6), fontStyle: FontStyle.italic),
                ),
              ],
            ),
          ),
  ...
  String _getDimensionName(TimelineDimension d) {
    switch (d) {
      case TimelineDimension.daily: return '日';
      case TimelineDimension.weekly: return '周';
      case TimelineDimension.monthly: return '月';
      case TimelineDimension.yearly: return '年';
    }
  }
  ...
  Widget _buildTimelineFlow(ColorScheme cs) {
    ...
    return Column(children: _events.map((e) => _buildJournalEntry(e, cs)).toList());
  }
  ...
  Widget _buildJournalEntry(TimelineEvent event, ColorScheme cs) {
    ...
    final isImportant = event.type == TimelineEventType.pomodoroEnd || event.type == TimelineEventType.todoCompleted;
    return Padding(
      padding: const EdgeInsets.only(bottom: 24.0),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(timeStr, style: TextStyle(fontSize: 12, fontFamily: 'monospace', color: cs.onSurfaceVariant.withValues(alpha: 0.5))),
          const SizedBox(width: 20),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(_formatEventTitle(event), style: TextStyle(fontSize: 15, fontWeight: isImportant ? FontWeight.w600 : FontWeight.normal, color: cs.onSurface)),
                if (event.subtitle != null) Text(event.subtitle!, style: TextStyle(fontSize: 13, color: cs.onSurfaceVariant.withValues(alpha: 0.7))),
              ],
            ),
          ),
        ],
  ...
  Widget _buildRangeSummaryCard(String title, String content, IconData icon, Color color, {String? subtitle, required ColorScheme cs}) {
    return Container(
  ...
  Widget _buildReflection(ColorScheme cs) {
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.format_quote_rounded, color: cs.primary.withValues(alpha: 0.3), size: 40),
          const SizedBox(height: 16),
          Text(
            _getReflectionText(), 
            textAlign: TextAlign.center, 
          ...
          const SizedBox(height: 24),
          Text('— 时光记录员', style: TextStyle(fontSize: 12, color: cs.primary.withValues(alpha: 0.5), letterSpacing: 2)),
        ],
      ),
    );
  }
  ...
  String _formatEventTitle(TimelineEvent event) {
    ...
    // Keyword detection
    final bool isHomework = sub.contains('作业') || sub.contains('练习') || sub.contains('刷题');
    final bool isExam = sub.contains('考试') || sub.contains('测验') || sub.contains('考证') || sub.contains('竞赛');
    final bool isReview = sub.contains('复习') || sub.contains('总结') || sub.contains('整理') || sub.contains('笔记');
    final bool isReading = sub.contains('阅读') || sub.contains('看书') || sub.contains('读书') || sub.contains('文献');

    switch (event.type) {
      case TimelineEventType.pomodoroStart: 
        if (isHomework) return pick(['开启作业攻坚模式', '开始在题海中远航', '正在消灭今日作业', '笔尖下的专注力', '让作业也变成享受']);
        if (isExam) return pick(['进入考前冲刺状态', '模拟实战，严阵以待', '为了那个目标，出发', '正在构建知识盔甲', '考场如战场，此刻亮剑']);
        if (isReview) return pick(['温故而知新', '梳理凌乱的思想', '搭建知识的脚手架', '让记忆变得更深邃', '正在进行深度总结']);
        if (isReading) return pick(['潜入书香的世界', '跨越时空的对话', '在文字间寻找答案', '静心享受阅读时光', '开启心灵的洗礼']);
        return pick(['开启了专注之旅', '让世界安静下来', '潜入思考的深处', '专注于当下的美好', '启动高效引擎', '此刻，唯有专注', '与目标同行', '听，是专注的声音', '开始雕琢时光', '进入心流频道']);
        
      case TimelineEventType.pomodoroEnd: 
        if (isHomework) return pick(['搞定了！离完成又近一步', '笔耕不辍，收获满满', '又攻克了一道难题', '让作业见证你的成长', '今日份作业能量注入完毕']);
        if (isExam) return pick(['离梦想的终点更近了', '多一分专注，多一分底气', '胜算在一点点积攒', '你的努力，必有回响', '为了那张完美的答卷']);
        if (isReview) return pick(['知识点已经刻入脑海', '思想变得更加清晰', '又完成了一次查漏补缺', '梳理，是为了更好的出发', '收获了成长的复利']);
        if (isReading) return pick(['收获了文字的芬芳', '拓宽了生命的维度', '与作者共鸣的瞬间', '知识的种子正在发芽', '精神世界又丰盈了一些']);
        return pick(['收获了专注的果实', '完成了一次深呼吸', '时光不负苦心人', '专注画上圆满句号', '又一次战胜了分心', '内心的充盈感', '让努力被看见', '合上专注的篇章', '达成一次深度连接', '汗水凝结成光']);
        
      case TimelineEventType.todoCreated: 
        return pick(['播种了一个新希望', '建立了奋斗的目标', '写下未来的约定', '开启新任务的挑战', '清单上的新成员', '勾勒成长的蓝图', '给未来一个交代', '新的旅程由此开始', '记录下这一刻的灵感', '待办项+1']);
        
      case TimelineEventType.todoCompleted: 
        if (isHomework) return pick(['彻底消灭了这项作业', '作业清单-1，快乐值+1', '笔尖圆满收官', '让成就感在纸面跳跃', '搞定，今晚可以睡个好觉']);
        if (isExam) return pick(['这是一次漂亮的突围', '又翻过了一座大山', '离证书/高分又近了', '为这场博弈画上句号', '你是自己的冠军']);
        return pick(['达成了一个小目标', '划掉了一个烦恼', '离梦想又近了一步', '又一个目标被征服', '完美的句点', '成就感满满', '坚持终有回响', '实力证明了自己', '完成，是最好的奖励', '给努力一个回馈']);
        
      case TimelineEventType.courseStart: 
        return pick(['步入知识的殿堂', '与智慧有个约会', '课堂，是成长的阶梯', '吸收新知识的养分', '聆听思想的碰撞', '在学海中起航', '开启学术模式', '知识点正在加载', '拓宽认知的边界', '在课堂遇见更好的自己']);
  ...
  String _getReflectionText() {
    final int seed = _selectedDate.day + _selectedDate.month + _selectedDate.year + _dimension.index;
    String pick(List<String> options) => options[seed % options.length];
  ...
  String _getMotivationText() {
    final int seed = _selectedDate.day + _selectedDate.month + _selectedDate.year;
    String pick(List<String> options) => options[seed % options.length];
