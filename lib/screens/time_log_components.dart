...
class _DayView extends StatefulWidget {
  final DateTime date; final bool crossDay;
  ...
  const _DayView({
    required this.date, required this.crossDay, required this.logs,
    required this.pomodoros, required this.tags, required this.onBack,
    required this.onCrossDayChanged, required this.onSaveLog,
    required this.onPomodoroTap, required this.onDeleteLog});

  @override State<_DayView> createState() => _DayViewState();
}
...
class _DayViewState extends State<_DayView> {
  ...
  void _openEntry() {
    ...
    showModalBottomSheet(
        ...
        builder: (_) => _LogEntrySheet(
            initialStart: st, initialEnd: en, initialTagId: _selectedTagId,
            tags: widget.tags,
        ...
        return Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Text('当天补录', style: TextStyle(fontSize: 16,
                      fontWeight: FontWeight.w700, color: _TC.text(ctx))),
                  ...
                  const Spacer(),
                  IconButton(icon: Icon(Icons.close, size: 20,
                      color: _TC.textHint(ctx)), onPressed: () => Navigator.pop(ctx)),
                ]),
                const SizedBox(height: 12),
                if (dayLogs.isEmpty)
                  Padding(padding: const EdgeInsets.symmetric(vertical: 24),
                      child: Center(child: Text('暂无补录记录',
                          style: TextStyle(color: _TC.textHint(ctx)))))
                else Flexible(child: ListView.separated(
                  shrinkWrap: true, itemCount: dayLogs.length,
                  ...
                  itemBuilder: (ctx2, i) {
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(color: _TC.inputFill(ctx2),
                          border: Border(left: BorderSide(color: c, width: 3)),
                          borderRadius: BorderRadius.circular(10)),
                      child: Row(children: [
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(log.title.isNotEmpty ? log.title : (tag?.name ?? '补录记录'),
                              ...
                              Row(children: [
                                if (tag != null) ...[
                                  ...
                                  Text(tag.name, style: TextStyle(fontSize: 10, color: c,
                                      fontWeight: FontWeight.w600)),
                                  const SizedBox(width: 6),
                                ],
                                Text(
                                    '${DateFormat('HH:mm').format(s)} → ${DateFormat('HH:mm').format(e)}',
                                    style: TextStyle(fontSize: 10, color: _TC.textHint(ctx2))),
                              ]),
                            ])),
                        const SizedBox(width: 8),
                        Text('${dur}min', style: TextStyle(fontSize: 14,
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            showDialog(context: ctx, builder: (dCtx) => AlertDialog(
                              ...
                              title: Text('删除记录', style: TextStyle(
                                  color: _TC.text(ctx), fontSize: 16)),
                              content: Text(
                                  '确定删除「${log.title.isNotEmpty ? log.title : (tag?.name ?? '补录记录')}」吗？',
                                  style: TextStyle(color: _TC.textSub(ctx), fontSize: 13)),
                              actions: [
                                TextButton(onPressed: () => Navigator.pop(dCtx),
                                    child: Text('取消',
                                        style: TextStyle(color: _TC.textSub(ctx)))),
                                TextButton(onPressed: () {
                                  Navigator.pop(dCtx);
                                  widget.onDeleteLog(log.id);
                                  if (dayLogs.length <= 1) Navigator.pop(ctx);
                                  else setModal(() {});
  ...
  Widget build(BuildContext context) {
    final gsMs  = _gridStart.millisecondsSinceEpoch;
    final geMs  = gsMs + 86400000;
    final dLogN = widget.logs.where((l) => l.endTime > gsMs && l.startTime < geMs).length;
      ...
      Expanded(child: Row(children: [
        Expanded(child: LayoutBuilder(builder: (ctx, c) {
          ...
          return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            SizedBox(width: kTimeAxisW, height: c.maxHeight,
                child: Stack(clipBehavior: Clip.none,
                    children: List.generate(24, (h) => Positioned(
                        top: h * hourH - 8, right: 8,
                        child: Text('${h.toString().padLeft(2, '0')}:00',
                            style: TextStyle(fontSize: 9,
                                color: _TC.timeLabel(ctx, major: h % 6 == 0),
      ...
      if (dLogN > 0) GestureDetector(
          ...
          child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(Icons.list_alt_outlined, size: 12, color: Color(0xFF60A5FA)),
                const SizedBox(width: 4),
                Text('已补录 $dLogN',
  ...
  Widget _buildGrid(double hourH, double totalH) {
    return LayoutBuilder(builder: (ctx, c) {
      ...
      final selTag = widget.tags.cast<PomodoroTag?>()
          .firstWhere((t) => t?.uuid == _selectedTagId, orElse: () => null);

      return GestureDetector(
        onPanStart:  (d) { final i = _getIndex(d.localPosition, w, hourH);
        if (i != null) setState(() { _dragStart = i; _dragEnd = i; }); },
        onPanUpdate: (d) { final i = _getIndex(d.localPosition, w, hourH);
        if (i != null) setState(() => _dragEnd = i); },
        onTapDown:   (d) { final i = _getIndex(d.localPosition, w, hourH);
        if (i != null) setState(() { _dragStart = i; _dragEnd = i; }); },
        child: Stack(children: [
          SizedBox(height: totalH, child: CustomPaint(size: Size(w, totalH),
              painter: _DayGridPainter(minutesPerBlock: _minutesPerBlock,
                  dragStart: _dragStart, dragEnd: _dragEnd,
                  logs: widget.logs, tags: widget.tags, gridStart: _gridStart,
                  selColor: selTag != null
                      ? hexColor(selTag.color, opacity: 0.48)
                      : accent.withOpacity(0.45),
                  isDark: isDark, hourH: hourH))),
  ...
  List<Widget> _pomodoroTapTargets(double width, double hourH) {
    ...
    for (final pom in widget.pomodoros) {
      ...
      final rs = max(pom.startTime, gsMs); final re = min(pe, geMs);
      final startF = (rs - gsMs) / 3600000; final endF = (re - gsMs) / 3600000;
      int sH = startF.floor(); int eH = endF.floor();
      ...
      for (int h = sH; h <= eH; h++) {
        final hS = max(h.toDouble(), startF); final hE = min((h + 1).toDouble(), endF);
        final left = (hS - h) * width; final right = (hE - h) * width;
        final bw = right - left; if (bw <= 0) continue;
        targets.add(Positioned(top: h * hourH + 1, left: left + 1, width: bw - 1,
            height: hourH - 2,
            child: GestureDetector(onTap: () => widget.onPomodoroTap(pom),
                behavior: HitTestBehavior.opaque,
                child: Container(decoration: BoxDecoration(color: fill,
                    borderRadius: BorderRadius.circular(4),
                    border: Border(left: BorderSide(color: mark, width: 3))),
                    child: bw > 20
  ...
  Widget _buildTagSidebar() => Container(width: 78, color: _TC.topBar(context),
      child: Column(children: [
        Padding(padding: const EdgeInsets.symmetric(vertical: 10),
            child: Text('TAGS', style: TextStyle(fontSize: 8,
                color: _TC.textHint(context), letterSpacing: 2))),
        Expanded(child: ListView.builder(itemCount: widget.tags.length,
            itemBuilder: (ctx, i) {
              final t   = widget.tags[i];
              final c   = hexColor(t.color);
              final sel = _selectedTagId == t.uuid;
              return GestureDetector(onTap: () => setState(() => _selectedTagId = t.uuid),
      ...
      child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min, children: [
              Text('$_durMin 分钟', style: TextStyle(fontSize: 16,
        ...
        TextButton(onPressed: () => setState(() { _dragStart = null; _dragEnd = null; }),
            child: Text('取消', style: TextStyle(color: _TC.textSub(context)))),
        const SizedBox(width: 8),
        ElevatedButton(onPressed: _openEntry,
...
class _DayGridPainter extends CustomPainter {
  final int minutesPerBlock; final int? dragStart, dragEnd;
  final List<TimeLogItem> logs; final List<PomodoroTag> tags;
  final DateTime gridStart; final Color selColor;
  final bool isDark; final double hourH;

  _DayGridPainter({required this.minutesPerBlock, this.dragStart, this.dragEnd,
    required this.logs, required this.tags, required this.gridStart,
    required this.selColor, required this.isDark, required this.hourH});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int c = 0; c <= bpr; c++) {
      canvas.drawLine(Offset(c * bw, 0), Offset(c * bw, size.height),
          c % 4 == 0 ? mj : mn);
    }

    for (final log in logs) {
      if (log.endTime <= gsMs || log.startTime >= geMs) continue;
      final sMs  = max(log.startTime, gsMs); final eMs = min(log.endTime, geMs);
    ...
    if (dragStart != null && dragEnd != null)
      _fill(canvas, min(dragStart!, dragEnd!), max(dragStart!, dragEnd!), bpr, bw,
  ...
  @override bool shouldRepaint(covariant _DayGridPainter o) => true;
}
...
class _LogEntrySheetState extends State<_LogEntrySheet> {
  late DateTime _start, _end; late String? _tagId;
  ...
  Future<void> _pickDate(bool isStart) async {
    final d = await showDatePicker(context: context,
        initialDate: isStart ? _start : _end, firstDate: DateTime(2020),
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(20)),
                  child: Text(_durLabel(), style: const TextStyle(fontSize: 12,
                      color: Color(0xFF4ADE80), fontWeight: FontWeight.w600))),
            ]),
            const SizedBox(height: 16),
            Text('CATEGORY', style: TextStyle(fontSize: 9, color: _TC.textHint(context),
                letterSpacing: 2)),
            const SizedBox(height: 8),
            Wrap(spacing: 8, runSpacing: 6, children: widget.tags.map((t) {
              final c = hexColor(t.color); final sel = _tagId == t.uuid;
              return GestureDetector(onTap: () => setState(() { _tagId = t.uuid; _tc.text = t.name; }),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 150),
            ...
            const SizedBox(height: 16),
            Text('TITLE', style: TextStyle(fontSize: 9, color: _TC.textHint(context),
                letterSpacing: 2)),
            ...
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: _TimePickerCard(label: '开始时间', dt: _start,
                  onTapTime: () => _pickTime(true), onTapDate: () => _pickDate(true))),
              const SizedBox(width: 10),
              Expanded(child: _TimePickerCard(label: '结束时间', dt: _end,
                  onTapTime: () => _pickTime(false), onTapDate: () => _pickDate(false))),
            ]),
...
class _TimePickerCard extends StatelessWidget {
  final String label; final DateTime dt;
  final VoidCallback onTapTime, onTapDate;
  const _TimePickerCard({required this.label, required this.dt,
    required this.onTapTime, required this.onTapDate});
  @override Widget build(BuildContext context) =>
      GestureDetector(onTap: onTapTime,
          child: Container(padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: _TC.inputFill(context),
                  border: Border.all(color: _TC.divider(context)),
                  borderRadius: BorderRadius.circular(12)),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(label, style: TextStyle(fontSize: 9, color: _TC.textHint(context),
                    letterSpacing: 1)),
                const SizedBox(height: 4),
                GestureDetector(onTap: onTapDate,
                    child: Text(DateFormat('MM月dd日').format(dt),
                        style: TextStyle(fontSize: 11, color: _TC.textSub(context)))),
                const SizedBox(height: 4),
                Row(children: [
                  Text(DateFormat('HH:mm').format(dt), style: TextStyle(fontSize: 22,
                      fontWeight: FontWeight.w700, color: _TC.text(context))),
                  const SizedBox(width: 6),
      ...
      child: Column(mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              IconButton(icon: Icon(Icons.close, color: _TC.textHint(context), size: 20),
            ...
            Text('RECENT RECORDS', style: TextStyle(fontSize: 9,
                color: _TC.textHint(context), letterSpacing: 2)),
            const SizedBox(height: 8),
            ConstrainedBox(constraints: const BoxConstraints(maxHeight: 220),
                child: ListView.separated(shrinkWrap: true, itemCount: allRecs.length,
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(color: _TC.inputFill(ctx),
                            border: Border(left: BorderSide(
                                color: r.isPomodoro ? Colors.redAccent : c, width: 3)),
                            borderRadius: BorderRadius.circular(8)),
                        child: Row(children: [
                          if (r.isPomodoro) ...[
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(r.title, style: TextStyle(fontSize: 12,
                                    color: _TC.text(ctx), fontWeight: FontWeight.w600)),
...
class _TagManagerSheet extends StatefulWidget {
  ...
  @override State<_TagManagerSheet> createState() => _TagManagerSheetState();
}
...
class _TagManagerSheetState extends State<_TagManagerSheet> {
  ...
  @override void initState() {
    super.initState();
    _list = widget.tags.map((t) =>
        PomodoroTag(uuid: t.uuid, name: t.name, color: t.color)).toList();
  }
  ...
  void _add() {
    ...
    setState(() {
      _list.add(PomodoroTag(
          ...
          name: _nc.text.trim(), color: _newColor));
      _nc.clear();
    });
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Text('标签管理', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700,
                  color: _TC.text(context))),
              const Spacer(),
              IconButton(icon: Icon(Icons.close, color: _TC.textHint(context), size: 20),
                  onPressed: () => Navigator.pop(context)),
            ...
            const SizedBox(height: 12),
            ConstrainedBox(constraints: const BoxConstraints(maxHeight: 260),
                child: ListView.builder(shrinkWrap: true, itemCount: _list.length,
                    itemBuilder: (ctx, i) {
                      final t = _list[i]; final c = hexColor(t.color);
                          ...
                          child: Row(children: [
                            ...
                            Expanded(child: TextField(
                                controller: TextEditingController(text: t.name)
                                  ..selection = TextSelection.collapsed(offset: t.name.length),
                                onChanged: (v) => _list[i] =
                                    PomodoroTag(uuid: t.uuid, name: v, color: t.color),
                                ...
                                decoration: InputDecoration(isDense: true, filled: true,
                                    ...
                                    border: OutlineInputBorder(
                                        borderRadius: BorderRadius.circular(8),
                                        borderSide: BorderSide(color: _TC.divider(ctx))),
                            ...
                            const SizedBox(width: 6),
                            Wrap(spacing: 3, children: kPalette.map((pc) =>
                                GestureDetector(onTap: () => setState(() =>
                                _list[i] = PomodoroTag(uuid: t.uuid, name: t.name, color: pc)),
                                    child: Container(width: 14, height: 14,
                                        decoration: BoxDecoration(color: hexColor(pc),
                                            shape: BoxShape.circle,
                                            border: Border.all(
                                                color: t.color == pc ? Colors.white : Colors.transparent,
                                                width: 1.5))))).toList()),
                            const SizedBox(width: 6),
                            GestureDetector(onTap: () => setState(() => _list.removeAt(i)),
                                child: Icon(Icons.delete_outline,
                                    color: Colors.red.withOpacity(0.55), size: 18)),
                          ]));
                    })),
            Divider(color: _TC.divider(context), height: 24),
            Text('NEW TAG', style: TextStyle(fontSize: 9, color: _TC.textHint(context),
                letterSpacing: 2)),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: TextField(controller: _nc, onSubmitted: (_) => _add(),
              ...
              ElevatedButton(onPressed: _add,
                  style: ElevatedButton.styleFrom(backgroundColor: accent.withOpacity(0.15),
                      foregroundColor: accent, elevation: 0,
            ...
            Wrap(spacing: 6, children: kPalette.map((pc) {
              ...
              return GestureDetector(onTap: () => setState(() => _newColor = pc),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                      width: 24, height: 24,
                      decoration: BoxDecoration(color: cc, shape: BoxShape.circle,
                          border: Border.all(
                              color: _newColor == pc ? Colors.white : Colors.transparent,
                              width: 2),
            ...
            const SizedBox(height: 16),
            SizedBox(width: double.infinity, height: 48,
                child: ElevatedButton(onPressed: () => Navigator.pop(context, _list),
                    style: ElevatedButton.styleFrom(backgroundColor: accent,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                        elevation: 0),
