...
class TeamGanttWidget extends StatelessWidget {
  ...
  const TeamGanttWidget(
      {super.key, required this.todos, this.viewDays = 30, this.onTodoTap});

  @override
  Widget build(BuildContext context) {
    final datedTodos =
        todos.where((t) => t.dueDate != null && !t.isDone).toList();
    // 🚀 按截止时间升序排序，确保甘特图从早到晚排列
    datedTodos.sort((a, b) => a.dueDate!.compareTo(b.dueDate!));

    if (datedTodos.isEmpty) {
      return const Center(
          child: Text("暂无带排期的活跃任务",
              style: TextStyle(fontSize: 10, color: Colors.grey)));
    }

    // 确定时间跨度
    final now = DateTime.now();
    final timelineStart = DateTime(now.year, now.month, now.day)
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text(
                  "任务线性排期 (${viewDays == 30 ? '月视图' : viewDays == 14 ? '双周' : '周视图'})",
                  style: const TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
                      color: Colors.grey)),
              Text("${datedTodos.length} 活跃",
            ...
            child: Column(
              ...
              children: [
                ...
                ...datedTodos.take(6).map((todo) {
                  ...
                  double startPos = start.difference(timelineStart).inMinutes /
                      (totalDays * 24 * 60).toDouble();
                  double duration = end.difference(start).inMinutes /
                      (totalDays * 24 * 60).toDouble();
                  ...
                  final color = todo.teamUuid != null
                        ...
                        Positioned(
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Text(_safeStr(todo.title),
                                  style: const TextStyle(
                                      color: Colors.white,
                                      fontSize: 8,
                                      fontWeight: FontWeight.bold),
