...
class _TurnstileVerificationWidgetState
  ...
  void _initWebView() {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
                ...
                if (mounted) {
                  if (isJson) {
                    ...
                    setState(() {
                      _hasError = true;
                      _errorMessage =
                          '人机验证页面配置错误，请检查 TURNSTILE_VERIFY_PAGE_URL';
                      _isLoadingPage = false;
                    });
          ...
          onWebResourceError: (WebResourceError error) {
            debugPrint(
                '[Turnstile] ❌ WebView error: ${error.description} (code: ${error.errorType})');
    ...
    if (defaultTargetPlatform != TargetPlatform.macOS) {
      controller.setBackgroundColor(widget.isDarkMode
    ...
    final url =
        '$_verifyPageUrl?sitekey=$siteKey&theme=$theme&action=${widget.action}';
  ...
  BoxDecoration _boxDecoration() {
    return BoxDecoration(
      ...
      color:
          widget.isDarkMode ? const Color(0x0DFFFFFF) : const Color(0xFFF8F8FF),
      border: Border.all(
        color: _isVerified
