...
团队数据。分类图标全部可自定义：系统预设分类只同步用户的图标覆盖项，分类名称、
类型和层级仍由内置定义保护；自定义分类按整条分类同步。系统默认付款方式仍只保留
在本地。旧服务端未声明 `finance_v1` 时，
...
八张表均包含 `pending_sync`。分类表另有 `icon_customized` 标记：系统默认分类
初始化时不排队，用户修改图标后才进入待同步队列。从旧版本升级时，已有个人记录会
安全排队一次，待服务端确认后清除；系统默认付款方式不会进入待同步队列。
