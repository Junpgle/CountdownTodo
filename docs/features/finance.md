...
当前状态：本地 MVP、第二阶段预算管理、第三阶段个人云同步和第四阶段自动化已实现。
...
在 SQLite 事务中幂等落库，同一周期不会重复记账。桌面端在记账首页使用
`Ctrl/Cmd + Shift + N` 打开快速录入。

第三阶段接入个人云同步：记账通过现有 `/api/sync` 认证和事务边界传输，使用独立
的 `finance_v1` 能力声明、个人水位线和增量游标。六类记账记录均按
...
不会跳过该修改。V48 起六张表增加 `pending_sync`：本地新增、编辑、删除和导入
先标记待同步，服务端确认接受后才清除；网络失败、冲突和请求期间的并发修改会
保留标记，因此增量上传不再依赖本机时间是否晚于云端游标。

## 代码结构

- `lib/features/finance/models/finance_models.dart`：交易、预算、周期规则、模板、分类、付款方式和汇总模型。
- `lib/features/finance/services/finance_storage.dart`：SQLite 表、系统默认数据、软删除、周期账单幂等生成和备份导入导出。
...
六张表均包含 `pending_sync`。从旧版本升级时，已有个人记录会安全排队一次，待
...
`finance` 数据块，包含交易、预算、分类和付款方式；CSV 导出用于当前月份的
人工分析。JSON 备份同时包含周期规则和快捷模板；预算页面可从记账页右上角进入，
自动化页面可从“自动化与快捷模板”进入。
