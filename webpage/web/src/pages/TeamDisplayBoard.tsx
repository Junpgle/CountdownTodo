...
const GanttChart: React.FC<{ todos: Todo[], dayWidth: number, onTaskClick: (task: Todo) => void }> = ({ todos, dayWidth, onTaskClick }) => {
  ...
  useEffect(() => {
    ...
    // 使用 ResizeObserver 监听容器宽度变化（解决移动端切换 Tab 后宽度从 0 变回正常导致无法定位的问题）
    if (scrollRef.current) {
...
const TeamDisplayBoard: React.FC<{ user: User; onBack?: () => void }> = ({ user, onBack }) => {
  ...
  const isDraggingCd = useRef(false);
  const startXCd = useRef(0);
  const scrollLeftCd = useRef(0);

  const handleCdMouseDown = (e: React.MouseEvent) => {
    isDraggingCd.current = true;
    startXCd.current = e.pageX - (cdScrollRef.current?.offsetLeft || 0);
    scrollLeftCd.current = cdScrollRef.current?.scrollLeft || 0;
    if (cdScrollRef.current) cdScrollRef.current.style.cursor = 'grabbing';
  };

  const handleCdMouseMove = (e: React.MouseEvent) => {
    if (!isDraggingCd.current || !cdScrollRef.current) return;
    e.preventDefault();
    const x = e.pageX - cdScrollRef.current.offsetLeft;
    const walk = (x - startXCd.current) * 1.5;
    cdScrollRef.current.scrollLeft = scrollLeftCd.current - walk;
  };

  const stopDraggingCd = () => {
    isDraggingCd.current = false;
    if (cdScrollRef.current) cdScrollRef.current.style.cursor = 'default';
  };
  ...
  return (
    <div className="h-screen w-screen text-white font-sans flex flex-col bg-[#020617] overflow-hidden">
      ...
      <header className="px-6 py-3 lg:px-10 lg:py-5 flex items-center border-b border-white/5 backdrop-blur-md shrink-0 z-50 relative">
        {/* 左侧：返回按钮 + PC端标题 */}
        <div className="flex items-center gap-4 z-10 sm:static absolute left-4">
          <button 
            onClick={() => { 
                if (onBack) onBack();
          ...
          <h1 className="hidden sm:block text-xl lg:text-4xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-white to-white/30 truncate max-w-[200px] lg:max-w-none">
            {selectedTeam?.name || '看板'}
          </h1>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center overflow-hidden px-4">
          {/* 移动端标题 */}
          <h1 className="sm:hidden text-xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-white to-white/30">
            {selectedTeam?.name || '看板'}
          </h1>
          ...
          <div 
            ref={cdScrollRef}
            onMouseDown={handleCdMouseDown}
            onMouseMove={handleCdMouseMove}
            onMouseUp={stopDraggingCd}
            onMouseLeave={stopDraggingCd}
          ...
          >
            {teamCountdowns.map(cd => {
              ...
              return (
                <div key={cd.uuid} className={`px-3 py-1 lg:px-6 lg:py-2 rounded-xl border flex flex-col items-center min-w-[100px] lg:min-w-[160px] shrink-0 transition-all hover:bg-white/5 ${isUrgent ? 'bg-red-500/10 border-red-500/30' : 'bg-blue-500/10 border-blue-500/20'}`}>
                  <span className={`text-[7px] lg:text-[9px] font-black uppercase tracking-widest truncate w-full text-center ${isUrgent ? 'text-red-400' : 'text-blue-400'}`}>{cd.title}</span>
                  <span className={`text-sm lg:text-xl font-black tabular-nums ${isUrgent ? 'text-red-400' : 'text-white'}`}>
                    {Math.max(0, days)} <span className="text-[10px] opacity-40">天</span>
                  </span>
                </div>
              );
            })}
