...
const PomodoroStatsView = ({ userId, todos }: { userId: number, todos: TodoItem[] }) => {
  ...
  const [filterTag, setFilterTag] = useState<string | 'all'>('all'); // Tag UUID
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // 默认拉取过去一年的数据作为基础数据集，也可根据需要调整 from/to 参数
      const fromMs = new Date(new Date().getFullYear() - 1, 0, 1).getTime();
      const recordsData = await ApiService.request(`/api/pomodoro/records?user_id=${userId}&from=${fromMs}`, { method: 'GET' });
...
export const WebApp = ({ onBack, user, onLogout }: { onBack: () => void, user: User, onLogout: () => void }) => {
  const [currentTab, setCurrentTab] = useState<'dashboard' | 'screentime' | 'pomodoro'>('dashboard');
  ...
  return (
      <div className="min-h-[100dvh] lg:h-[100dvh] bg-slate-50 flex flex-col font-sans selection:bg-indigo-500 selection:text-white lg:overflow-hidden">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="hidden sm:flex items-center gap-1">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 ${currentTab === 'pomodoro' ? 'bg-amber-50 text-amber-700' : 'text-slate-500 hover:bg-slate-50'}`}
                >
        ...
        <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-40 px-4 py-2 pb-safe flex justify-around">
          ...
          <button onClick={() => {setCurrentTab('pomodoro'); setMobileTab('home');}} className={`flex flex-col items-center gap-1 p-2 ${currentTab === 'pomodoro' ? 'text-amber-600' : 'text-slate-400'}`}>
        ...
        <main className="flex-1 max-w-[1600px] mx-auto w-full p-4 sm:p-6 lg:p-8 pb-24 sm:pb-8 flex flex-col min-h-0 overflow-y-auto lg:overflow-hidden">
          ...
          {currentTab === 'pomodoro' && mobileTab !== 'settings' && <PomodoroStatsView userId={user.id} todos={todos} />}
        </main>
